...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "New owner cannot be the current token holder");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner() || isApprovedForAll(to, msg.sender), "Only owner can transfer token unless it is approved for all");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(to != address(0), "ERC721: minting cannot happen to address(0)");
        require(!_exists(tokenId), "ERC721: the token must not be already minted");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "ERC721: Only the owner of a token can transfer it");

        // TODO: require token is being transfered to valid address
        require(to != address(0), "ERC721: Token cannot be transferred to address(0)");
